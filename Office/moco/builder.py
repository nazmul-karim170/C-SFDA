...
class AdaMoCo(nn.Module):
    ...
    def __init__(
        ...
        momentum_model,
        K=128,
        m=0.98,          ## Use 0.95 for DomainNet, 0.975 or more for VISDA-C 
        T_moco=0.07,
    ...
    def forward(self, im_q, im_k=None, cls_only=False):
        ...
        if cls_only:
            # with torch.no_grad():
            #     feats, logits = self.momentum_model(im_q, return_feats=True)
            ## change the feats_q to feats
